...
$id = $_GET['id'];	
$cor = 0;	
$qryStr = "SELECT origem FROM origens_recursos WHERE id = {$id}";
...
$q_delete = "DELETE FROM origens_recursos WHERE id = {$id};";
$r_delete = sql_executa($q_delete);

if($r_delete){
	$msg = "A origem de recursos {$row['origem']} foi excluída com sucesso!";
}else{
	$msg = "Não foi possível excluir a origem {$row['origem']}. Por favor, tente novamente.";
}
