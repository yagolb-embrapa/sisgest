...
if($submit){
	...
	$q_ver = "SELECT * FROM users WHERE login = '{$login}' AND id <> '{$id}'";
	$r_ver = sql_executa($q_ver);
	if(sql_num_rows($r_ver) > 0){
		$erros[] = 'login';
		$msg_erro = "Esse login já é utilizado por outro usuário.";	
	}
	...
	if(count($erros)>0){
		...
		if(count($erros)==1 && empty($msg_erro)){
		...
		}elseif(count($erros)==1 && !empty($msg_erro)){
			$msg_erro = "Esse login já é utilizado por outro usuário.";		
		}elseif(count($erros)>1 && !empty($msg_erro)){		
			$msg_erro = "Alguns campos não foram preenchidos corretamente e foram marcados com um asterisco vermelho. Por favor, verifique-os e tente novamente.<br>Esse login já é utilizado por outro usuário. ";
		}else{
		...
		$query = "UPDATE users SET nome='{$nome}', login='{$login}', nivel='{$nivel}' WHERE id = {$id};";						
		$result = sql_executa($query);
		//mensagem de sucesso		
		if($result){
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#EFFFF4'>
					<td align='center'><span align='center' style='color:#296F3E;'>Usuário editado com sucesso!</span></td>
				</tr>
			</table>		
			<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
			...
			echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#FFEFEF'>
					<td align='center'><span align='center' style='color:red;'>Erro ao salvar.</span></td>
				</tr>
			</table>		
		<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
		}
	...
	if($id){
		$q_rec = "SELECT * FROM users WHERE id = {$id}";
		$r_rec = sql_executa($q_rec);
		if(sql_num_rows($r_rec)>0){
			$campo = sql_fetch_array($r_rec);
			$nome = $campo['nome'];
			$login = $campo['login'];
			$nivel = $campo['nivel'];		
		}else{
	...
	if($flag_erro == 1){
		echo "<table width='100%' style='border:1px solid black;' bgcolor='' cellspacing='0' cellpadding='5' height='50px'>						
				<tr bgcolor='#FFEFEF'>
					<td align='center'><span align='center' style='color:red;'>Usuário não encontrado.</span></td>
				</tr>
			</table>		
		<div align='center' style='margin: 0 0 25px 0; padding: 2px 2px 2px 2px;'></div>";
		exit();	
